...
class minesweeper(commands.Cog):
    ...
    async def boom(self, ctx, columns = None, rows = None, bombs = None):
        ...
        #embed = discord.Embed(title='\U0001F635 Сапёр \U0001F635', color=0xff2400)
        #embed.add_field(name='Столбцов:', value=columns, inline=True)
        #embed.add_field(name='Строк:', value=rows, inline=True)
        #embed.add_field(name='Площадь поля:', value=columns * rows, inline=True)
        #embed.add_field(name='\U0001F4A3 Бомб:', value=bombs, inline=True)
        #embed.add_field(name='\U0001F4A3 Процентов:', value=f'{percentage}%', inline=True)
        #embed.add_field(name='По запросу:', value=ctx.author.display_name, inline=True)
